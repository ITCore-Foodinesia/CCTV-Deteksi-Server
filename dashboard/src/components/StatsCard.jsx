...
const StatsCard = ({ icon: Icon, label, value, badge, bgColor, iconColor, badgeColor, compact = false, isAnimated = false }) => {
  return (
    <div className={`${bgColor} border ${compact ? 'p-3 rounded-xl' : 'p-4 rounded-[1.5rem]'} flex flex-col relative overflow-hidden group`}>
      ...
      <div className={`${compact ? 'mb-1' : 'mb-2'}`}>
        <Icon className={`${compact ? 'w-6 h-6' : 'w-8 h-8'} ${iconColor} ${isAnimated ? 'animate-spin' : ''}`} />
      </div>
      <span className={`${compact ? 'text-[9px]' : 'text-[10px]'} font-bold ${iconColor} uppercase tracking-wider mb-1`}>
        {label}
      </span>
      <span className={`${compact ? 'text-base sm:text-lg' : 'text-3xl'} font-black ${iconColor.replace('text-', 'text-').replace('-600', '-800')} truncate`}>
        {value}
      </span>
